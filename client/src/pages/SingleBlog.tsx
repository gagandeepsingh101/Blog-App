        ...
        <div className=' w-full h-full flex flex-col justify-center items-center py-28 gap-5 overflow-y-scroll'>
            <div className='flex w-11/12 mx-auto items-center justify-between'>
                ...
                <div className='w-1/5 flex gap-2 justify-end'>
                    {userId !== blogData.authorId ? (
                        <BiBookmark onClick={() => dispatch(addBookMarkBlogs(blogData))} className="text-3xl text-green-300 hover:text-green-600 cursor-pointer" />
                    ) : (
                        <>
            ...
            <div className='flex gap-3 w-5/6'>
                {(blogData?.category as string[])?.map((categoryItem: string, idx: number) => (
                    <span key={idx} className={`max-w-32 flex items-center justify-between gap-2 px-3 py-2 rounded-xl truncate ${idx === 0 ? "bg-red-200" : idx === 1 ? "bg-yellow-200" : idx === 2 ? "bg-blue-200" : ""}`}>
                        {categoryItem}
                    </span>
                ))}
            </div>
            <img src={blogData.image} className='max-w-full h-4/5 rounded-xl ring-blue-500 ring-1 p-1 mx-auto' alt="" />
            <p className='w-11/12 mx-auto font-bold flex flex-col justify-between gap-1  text-gray-600 md:w-8/12  md:flex-row lg:text-xl '><span className='hover:underline hover:text-black'>@ {blogData.authorName}</span> <span>{formatDate(blogData.createdAt.toString())}</span></p>
