...
const HeaderMenuBox = ({ isOpen, setIsOpen }: {
    ...
    return (
        <>
            ...
            {/* Menu content */}
            {isOpen && (
                <div className=' w-1/2 h-48 right-0 top-14 bg-white absolute  border-2 shadow-md flex flex-col justify-evenly rounded-md   z-50 p-2 md:w-1/4 lg:h-64 lg:w-1/6 lg:top-[70px] lg:right-16'>
                    {/* Profile link */}
                    <p onClick={() => {
                        navigate("/profile");
                        setIsOpen(!isOpen);
